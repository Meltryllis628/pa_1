...
def main(args):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while v1 and v2 and v3:
            ...
            org_sym_key_byte = gen_sym_key()
            sym_key = Fernet(org_sym_key_byte)
            sym_key_byte = encrypt(org_sym_key_byte,pub)[0]
            sym_key_len = len(sym_key_byte)
            s.sendall(convert_int_to_bytes(sym_key_len))
            s.sendall(sym_key_byte)
            ...
            with open(filename, mode="rb") as fp:
                data = fp.read()
            encrypted_data = sym_key.encrypt(data)
            s.sendall(convert_int_to_bytes(1))
            s.sendall(convert_int_to_bytes(len(encrypted_data)))
            s.sendall(encrypted_data)
            # block_list = encrypt(data,pub)
            # s.sendall(convert_int_to_bytes(1))
            # s.sendall(convert_int_to_bytes(len(block_list)))
            # for block in block_list:
            #     s.sendall(convert_int_to_bytes(len(block)))
            #     s.sendall(block)
